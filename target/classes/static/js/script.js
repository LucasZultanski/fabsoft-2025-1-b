const screens = ['homeScreen', 'checkinScreen', 'avaliacaoScreen', 'notificacaoScreen', 'adminLoginScreen', 'adminDashboardScreen', 'adminPostEventScreen', 'eventosScreen'];
...
function showScreen(screenId) {
    ...
    // Limpa campos do check-in ao sair da tela de check-in
    if (screenId !== 'checkinScreen') {
        const checkinNameInput = document.getElementById('checkinName');
        const checkinCpfInput = document.getElementById('checkinCpf');
        ...
        if (checkinCpfInput) checkinCpfInput.value = '';
        if (checkinAgeInput) checkinAgeInput.value = '';
        if (checkinGenderInput) checkinGenderInput.value = '';
        if (checkinStateInput) checkinStateInput.value = '';
        if (checkinCityInput) {
            checkinCityInput.value = '';
            checkinCityInput.disabled = true;
        }
        if (checkinEmailInput) checkinEmailInput.value = '';
        if (checkinPhoneInput) checkinPhoneInput.value = '';
    ...
    if (screenId === 'checkinScreen') {
        ...
        initBrazilianStates();
        checkinCityInput.disabled = true;
        checkinCityInput.innerHTML = '<option value="">Selecione a Cidade...</option>';
    }
    if (screenId === 'adminDashboardScreen') {
        fetchAdminEventos();
    }
    if (screenId === 'eventosScreen') {
        carregarEventos();
    }
}

function showScreenWithHash(screenId) {
    showScreen(screenId);
    const hash = screenHashMap[screenId];
...
async function submitCheckin() {
    ...
    clearFieldErrors([checkinNameInput, checkinCpfInput, checkinAgeInput, checkinGenderInput, checkinStateInput, checkinCityInput, checkinEmailInput, checkinPhoneInput]);

    let isValid = true;
    const email = checkinEmailInput.value.trim();
    const phone = checkinPhoneInput.value.trim();
    const cpf = checkinCpfInput.value.trim();
    ...
    if (!cpf || !cpfRegex.test(cpf)) {
        markFieldError(checkinCpfInput);
        isValid = false;
    }
    ...
    // Verifica se já existe check-in para o CPF no mesmo dia
    const hoje = new Date();
    const dataHoje = hoje.toISOString().slice(0, 10); // yyyy-mm-dd
    try {
        const verificaResp = await fetch(`/api/v1/checkups/exists?cpf=${encodeURIComponent(cpf)}&date=${dataHoje}`);
        if (verificaResp.ok) {
            const existe = await verificaResp.json();
            if (existe && existe.exists) {
                lastOperationWasErrorScreenId = 'checkinScreen';
                showModal('Check-in já realizado', 'Você já fez check-in hoje com este CPF. Só é permitido um check-in por dia.');
                return;
            }
        }
    } catch (e) {
        // Se não conseguir verificar, pode bloquear por segurança
        lastOperationWasErrorScreenId = 'checkinScreen';
        showModal('Erro de Verificação', 'Não foi possível verificar se já existe check-in para este CPF hoje. Tente novamente.');
        return;
    }
...
async function submitAvaliacao() {
    ...
    const avaliacaoCpfInput = document.getElementById('avaliacaoCpf');
    // Limpa erros visuais anteriores
    clearFieldErrors([avaliacaoCpfInput]);
    let isValid = true;
    const cpf = avaliacaoCpfInput.value.trim();
    const cpfRegex = /^\d{3}\.\d{3}\.\d{3}-\d{2}$|^\d{11}$/;
    if (!avaliacaoCpfInput || !cpf || !cpfRegex.test(cpf)) {
        markFieldError(avaliacaoCpfInput);
        isValid = false;
    }
    if (!isValid) {
        lastOperationWasErrorScreenId = 'avaliacaoScreen';
        showModal('Erro de Preenchimento', 'Por favor, insira um CPF válido para vincular sua avaliação.');
        return;
    }
    const currentDateTime = new Date().toISOString().replace('Z', '');
...
// Mapeamento de tela para hash
const screenHashMap = {
    'homeScreen': '',
    'checkinScreen': 'checkin',
    'avaliacaoScreen': 'avaliacao',
    'notificacaoScreen': 'notificacao',
    'adminLoginScreen': 'admin',
    'eventosScreen': 'eventos',
    // Adicione outros se necessário
};

// Roteia ao carregar a página ou mudar o hash
function handleHashChange() {
    const hash = window.location.hash.replace('#/', '');
    const screenId = Object.keys(screenHashMap).find(key => screenHashMap[key] === hash) || 'homeScreen';
    showScreen(screenId);
}
...
async function carregarEventos() {
    ...
    eventosList.innerHTML = '<div class="text-gray-500">Carregando eventos...</div>';
    try {
        const response = await fetch('/api/v1/eventos');
        if (!response.ok) throw new Error('Erro ao buscar eventos');
        const eventos = await response.json();
        if (!eventos.length) {
            eventosList.innerHTML = '<div class="text-gray-500">Nenhum evento cadastrado.</div>';
            return;
        }
        eventosList.innerHTML = eventos.map(evento => `
            <div class="bg-white shadow-md rounded-lg p-6 w-full max-w-2xl text-left">
                <h3 class="text-2xl font-bold text-blue-700 mb-2">${evento.nome || evento.titulo || 'Evento'}</h3>
                <p class="text-gray-600 mb-1"><b>Data/Hora:</b> ${evento.dataHora ? new Date(evento.dataHora).toLocaleString('pt-BR') : '-'}</p>
                <p class="text-gray-600 mb-1"><b>Local:</b> ${evento.local || '-'}</p>
                <p class="text-gray-700 mt-2">${evento.texto || ''}</p>
                ${evento.imagem ? `<img src="/eventos/${evento.imagem}" alt="Imagem do Evento" style="width:100%;height:auto;object-fit:contain;" class="mt-4 rounded shadow">` : ''}
            </div>
        `).join('');
    } catch (e) {
        eventosList.innerHTML = '<div class="text-red-500">Erro ao carregar eventos.</div>';
    }
}

async function fetchAdminEventos() {
    const tbody = document.getElementById('adminEventosTableBody');
    if (!tbody) return;
    tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4">Carregando eventos...</td></tr>';
    try {
        const response = await fetch('/api/v1/eventos');
        if (!response.ok) throw new Error('Erro ao buscar eventos');
        const eventos = await response.json();
        if (!eventos.length) {
            tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4">Nenhum evento para exibir.</td></tr>';
            return;
        }
        tbody.innerHTML = eventos.map(evento => `
            <tr>
                <td class="py-2 px-4 font-semibold">${evento.nome || evento.titulo || ''}</td>
                <td class="py-2 px-4">${evento.dataHora ? new Date(evento.dataHora).toLocaleString('pt-BR') : '-'}</td>
                <td class="py-2 px-4">${evento.dataFim ? new Date(evento.dataFim).toLocaleString('pt-BR') : '-'}</td>
                <td class="py-2 px-4">${evento.local || '-'}</td>
                <td class="py-2 px-4">
                    <button class="btn-submit bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded" onclick="deleteEvento(${evento.id})">Excluir</button>
                </td>
            </tr>
        `).join('');
    } catch (e) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4 text-red-500">Erro ao carregar eventos.</td></tr>';
    }
}
...
async function deleteEvento(id) {
    ...
    try {
        const response = await fetch(`/api/v1/eventos/${id}`, { method: 'DELETE' });
        if (response.status >= 200 && response.status < 300) {
            fetchAdminEventos();
            showModal('Evento Excluído', 'O evento foi excluído com sucesso.');
        } else {
            showModal('Erro', 'Não foi possível excluir o evento.');
        }
    } catch (e) {
        showModal('Erro', 'Erro ao tentar excluir o evento.');
    }
}
