...
public class CheckupController {
    ...
    public ResponseEntity<?> postCheckup(@Valid @RequestBody CheckinRequestDTO checkinRequestDTO) {
        ...
        // Remover validação de CPF, apenas verifica duplicidade de check-in
        String cpf = checkinRequestDTO.getUsuarioIdentifier();
    ...
    @GetMapping("/exists")
    public ResponseEntity<Map<String, Boolean>> existsCheckinByCpfAndDate(@RequestParam String cpf, @RequestParam String date) {
        try {
            java.time.LocalDate dia = java.time.LocalDate.parse(date);
            java.time.LocalDateTime startOfDay = dia.atStartOfDay();
            java.time.LocalDateTime endOfDay = dia.atTime(23, 59, 59);
            boolean exists = checkupService.existsCheckinByCpfAndDate(cpf, startOfDay, endOfDay);
