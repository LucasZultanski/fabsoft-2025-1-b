...
public class AvaliacaoController {
    ...
    public ResponseEntity<?> postAvaliacao(@RequestBody AvaliacaoRequestDTO request) {
        ...
        // Validação de CPF
        String cpf = request.getUsuarioIdentifier();
        if (cpf == null || cpf.trim().isEmpty() || !cpf.matches("^\\d{3}\\.\\d{3}\\.\\d{3}-\\d{2}$|^\\d{11}$")) {
            return new ResponseEntity<>(Map.of("message", "CPF inválido. Informe um CPF válido para avaliação."), HttpStatus.BAD_REQUEST);
        }
